...
# Store Submission Checklist — stikfix v1.1.1
...
- [ ] Zip the directory: `cd .output && zip -r stikfix-1.1.1-chrome.zip chrome-mv3/`
- [ ] Verify zip size is under 10 MB (CWS limit)

### Firefox build

- [ ] `npm run build:firefox` completes without errors
- [ ] Output at `.output/firefox-mv*/` — verify `manifest.json` and `browser_specific_settings.gecko.id` = `stikfix@stikfix.com`
- [ ] Zip the directory: `cd .output && zip -r stikfix-1.1.1-firefox.zip firefox-mv*/`
- [ ] Prepare a source code zip for AMO (required for bundled extensions): `git archive HEAD --format=zip -o stikfix-1.1.1-source.zip`
...
- [ ] Privacy policy is reachable at a public URL — recommended: `https://github.com/omernesh/stikfix/blob/main/store/privacy-policy.md`
...
- [ ] Choose "Add new item" and upload `stikfix-1.1.1-chrome.zip`
...
- [ ] **Privacy policy URL:** `https://github.com/omernesh/stikfix/blob/main/store/privacy-policy.md`
...
- [ ] Note explicitly: the companion host (`npx stikfix init`) must be installed before the extension can be fully exercised; link the npm package: https://www.npmjs.com/package/stikfix
...
- [ ] Upload `stikfix-1.1.1-firefox.zip`
- [ ] When prompted about source code: select "Yes, I have source code to submit" and upload `stikfix-1.1.1-source.zip`

### Listing fields (copy from `store/firefox-amo/listing.md`)

- [ ] **Name:** `stikfix`
- [ ] **Add-on ID:** `stikfix@stikfix.com` (must match `browser_specific_settings.gecko.id`)
- [ ] **Summary:** copy from listing.md
- [ ] **About this extension:** copy from listing.md → "About This Extension" section
- [ ] **Homepage:** `https://github.com/omernesh/stikfix`
- [ ] **Support email:** `omernesher@gmail.com`
- [ ] **Support site:** `https://github.com/omernesh/stikfix/issues`
- [ ] **License:** MIT
...
- [ ] AMO requires source code for minified/bundled extensions — upload `stikfix-1.1.1-source.zip`
...
- [ ] Note: companion host (`npx stikfix init`) must be installed before extension can be fully exercised
...
- [ ] `stikfix` npm package at https://www.npmjs.com/package/stikfix reflects the current version
- [ ] `npx stikfix init --root <path>` works on Windows, macOS, and Linux
...
3. **Companion host dependency** — Reviewers cannot fully exercise the note-capture flow without running `npx stikfix init`. The reviewer notes template in both listing files explains this. Offer to assist reviewers via the support email if needed.

...
export interface LauncherOptions {
   ...
   * When set, the launcher runs `"<hostExe>" serve --root "<root>"` instead of
   * `"node" "<hostEntryPath>" --root "<root>"`. Absent (npx/node install) →
   * exact prior `node hostEntry` launcher behavior.
   */
  hostExe?: string;
...
interface RegisterOptions {
  ...
  execReg?: (args: readonly string[]) => void;
  /**
   * When true, point the native-messaging manifest `path` DIRECTLY at
   * `hostBinPath` (the standalone stikfix-host.exe, which self-detects native
   * mode) and SKIP writing the node/.cjs launcher wrapper. Default false
   * preserves the existing npx/node behavior exactly (manifest points at a
   * per-OS wrapper that runs `node <abs cjs>`).
   */
  directPath?: boolean;
}
...
// ---------------------------------------------------------------------------
// teardownHost — shared uninstall sequence (native host + startup + config)
// ---------------------------------------------------------------------------
...
export interface TeardownResult {
  /** False only if unregisterNativeHost threw unexpectedly (it is idempotent internally). */
  manifestRemoved: boolean;
  manifestError?: string;
  /** Set only if unregisterStartup threw unexpectedly (it is idempotent internally). */
  startupError?: string;
  configPath: string;
 ...
 */
export function teardownHost(opts: TeardownOptions = {}): TeardownResult {
  const plat = opts.plat ?? process.platform;
  const home = opts.home ?? homedir();
  const browser = opts.browser ?? 'chrome';

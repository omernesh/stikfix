...
- **D-05:** Build **three** standalone, **unit-tested** reusable utilities (consumed by Phases 5/6):
  ...
  - **(b) double-`requestAnimationFrame` flush** — a helper that awaits **two** rAFs so the page/own-UI paints (e.g. hides stikfix UI, double-rAF, then capture) before the screenshot, so the shot is clean page pixels (PRD §7.3 "hide stikfix's own UI … then restore").

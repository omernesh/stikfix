...
### Host Discovery
- **D-03:** The SW probes **all** of ports 39240–39260 on `127.0.0.1` in parallel via `GET /status`, collecting every responder with `{app:'stikfix'}` into a registry `{project,port,token,origins}`. Runs on Review-Mode entry and on SW wake. Short per-probe timeout so dead ports fail fast.
...
- `PRD.md` §7.4 — Host discovery, routing & auth (probe range, resolution order, `X-Stikfix-Token`)
...
- `.planning/phases/02-host-mvp/02-CONTEXT.md` + `02-SUMMARY.md` — the host's `/status` (discovery handshake) + `/annotation` (token-gated) contracts the extension consumes; `X-Stikfix-Token` header
</canonical_refs>
...
### Reusable Assets
- `host/src/server.ts` — `GET /status` returns `{app,version,name,root,notesDir,origins}` (the exact discovery payload the SW parses) and `POST /annotation` is token-gated with `X-Stikfix-Token` + CORS echo-Origin. The extension is the client of these contracts — match them precisely.
...
- sfx-*/stikfix namespace; clean-room gate must stay green.

### Integration Points
- SW `fetch` → host `POST /annotation` with `X-Stikfix-Token` (this is the relay the phase proves).
...
- The dummy Send must produce a **visibly distinct** stub note (e.g. comment "stikfix relay proof") so it's obvious in the notes dir during verification and easy to delete.
